...
{
   simple_vector::simple_vector(int _n, int _k)
      : N(_n)
      , K(_k)
   {
   ...
   bool simple_vector::has_next()
   {
      return m_index[0] < (N-K+1);
   }

   std::vector<int> simple_vector::next()
   {
...
namespace combination_generator
{
   simple_pointer::simple_pointer(int _n, int _k)
      : N(_n)
      , K(_k)
   {
      // check values for N and K
   ...
   bool simple_pointer::has_next()
   {
      return m_index[0] < (N-K+1);
   }

   const int* simple_pointer::next()
   {
...
namespace combination_generator
{
   ping_pong::ping_pong(int _n, int _k)
      : N(_n)
      , K(_k)
      , m_return_A(true)
   {
...
namespace combination_generator
{
   single_pass::single_pass(int _n, int _k)
      : N(_n)
      , K(_k)
   {
      // check values for N and K
   ...
   std::vector<int> single_pass::next()
   {
      // Make a copy of index
      std::vector<int> to_return = m_index;
